...
1. **升级并适配新版本构建链**
   - **Rspack 2 适配**：修复 `@rspack/dev-server` 在 `tsx` 静态导入场景下内部 chunk 加载异常的问题，改为在启动渲染进程 Dev Server 时动态导入。
   - **Vue Router 5 适配**：移除导航守卫中已废弃的 `next()` 回调写法，改为返回值式守卫，消除运行时弃用警告。
   - **Vize 校验适配**：修正非 void HTML 元素自闭合写法，满足更严格的模板校验规则。

2. **Electron 42 类型兼容修复**
   - **打印配置类型收紧**：修复 `WebContentsPrintOptions` 中 `margins`、`pageSize` 类型更严格后产生的类型错误。
   - **打印机列表兼容**：移除对已不存在的 `PrinterInfo.isDefault` 字段依赖，避免新版 Electron 类型校验失败。

3. **渲染进程构建稳定性修复**
   - **Vue 单文件组件导出修复**：补齐布局组件的默认导出能力，解决 Rspack 严格 ESM 校验下 `default export` 缺失的问题。
   - **UnoCSS 接入边界调整**：明确 UnoCSS 仅作用于渲染进程构建，避免 Electron 多进程构建场景中样式提取边界不清晰。
   - **README 渲染修正**：修复 GitHub task list 语法，使功能列表可以正确渲染为复选框。
...
1. **全页面 UI 架构重构**
   - **全面迁移 Naive UI**：对应用内所有核心页面（浏览器、下载管理、打印预览、工具箱、设置等）进行了深度重构，统一采用 Naive UI 组件库，提升了视觉一致性与交互流畅度。
   - **模块化拆分**：优化了页面代码结构，引入了更细粒度的组件拆分，大幅提升了开发效率与代码可维护性。
   - **视觉动效升级**：为所有页面切换引入了平滑的 `fade-up` 转换动效，提升了应用的现代化感官。

2. **多窗口主题自动同步系统**
   - **全局状态共享**：基于 Pinia 与 Electron IPC 通讯机制，实现了多窗口间的主题状态（浅色/深色/跟随系统）实时同步。
   - **原生标题栏适配**：主题切换时，所有窗口的 Windows 原生控制按钮颜色（TitleBar Overlay）会自动根据背景色进行智能适配，确保视觉体验的完美统一。
   - **零延迟响应**：优化了主题切换逻辑，确保在主进程广播状态后，所有已打开的渲染进程窗口能瞬间响应并完成样式更新。

3. **新增现代化“关于”页面**
   - **品牌与环境信息**：独立展示应用品牌标识、版本号 (v1.0.0) 以及详细的系统运行环境（OS 平台、发布版本、架构等）。
   - **智能 Logo 系统**：Logo 图标会根据当前操作系统（Windows/macOS/Linux）自动切换为对应的平台标识。

4. **通用设置功能增强**
   - **语言一键切换**：支持中英文无缝切换，并实现了全局组件的 I18n 自动化关联。
   - **偏好持久化**：所有主题与语言设置均支持本地持久化存储，并在应用启动时自动恢复。

5. **底层优化与细节改进**
   - **等宽字体集成**：在系统信息、版本号等关键位置启用了等宽字体，确保排版整齐。
   - **布局优化**：改进了侧边栏与主区域的响应式布局，在不同窗口尺寸下均能保持良好的显示效果。
